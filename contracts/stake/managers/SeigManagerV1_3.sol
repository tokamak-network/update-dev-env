    ...
    SeigManagerV1_3Storage
{
    ...
    // function _estimatedDistribute(uint256 blockNumber, address layer2)
    //     internal
    //     view
    //     returns (
    //         uint256 maxSeig,
    ...
    //                 layer2Seigs =
    //                     ((templ2RewardPerUint * oldLayer2Info.layer2Tvl) / WEI_UNIT) - oldLayer2Info.initialDebt;
    //             }
    //         }
    //     }
    // }
    ...
    function _updateSeigniorage() internal ifFree returns (bool) {
        ...
        (nextTotalSupply, operatorSeigs, isCommissionRateNegative_) =
            _calcSeigsDistribution(msg.sender, coinage, prevTotalSupply, seigs, operator);

        // gives seigniorages to the layer2 as coinage
        require(coinage.setFactor(_calcNewFactor(prevTotalSupply, nextTotalSupply, coinage.factor())), "fail setFactor");
    ...
    function _calcSeigsDistribution(
        ...
        // β:
        uint256 delegatorSeigs = operatorRate == RAY ? operatorSeigs : FullMath.rdiv(operatorSeigs, RAY - operatorRate);
    ...
    function _totalSupplyOfTon(uint256 blockNumber) internal view returns (uint256 tos) {
        ...
        tos = initial + (_seigPerBlock * (blockNumber - startBlock)) - (OneAddressBalance * GWEI_UNIT) - burntAmount;
    }
}
