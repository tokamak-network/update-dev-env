    ...
    SeigManagerV1I
{
    ...
    uint256 constant WAD_UNIT = 1e18;
    ...
    function uncommittedStakeOf(address layer2, address account) public view returns (uint256) {
        ...
        uint256 uncommittedBalance =
            coinage.balanceOf(account).mulDivRoundingUp(RAY, prevFactor).mulDivRoundingUp(newFactor, RAY);
        ...
        returns (uint256 totAmount)
    {
        ...
        return
            (totBalalnce - coinageTotalSupply).mulDivRoundingUp(amount, RAY).mulDivRoundingUp(RAY, coinageTotalSupply);
    }
    ...
    function _calcSeigsDistribution(
        ...
        uint256 delegatorSeigs =
            operatorRate == RAY ? operatorSeigs : operatorSeigs.mulDivRoundingUp(RAY, RAY - operatorRate);
    ...
    function totalSupplyOfTon() public view returns (uint256 tos) {
        ...
        tos = initial + (_seigPerBlock * (block.number - startBlock)) - (ITON(_ton).balanceOf(address(1)) * (10 ** 9))
            - burntAmount;
    }
