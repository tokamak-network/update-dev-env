    ...
    SeigManagerV1_3Storage
{
     ...
     */
    event CommitLog1(
        uint256 totalStakedAmount, uint256 totalSupplyOfWTON, uint256 prevTotalSupply, uint256 nextTotalSupply
    );
    ...
    function initialize(
        ...
        emit Initialized(ton_, wton_, registry_, depositManager_, seigPerBlock_, factory_, lastSeigBlock_);
    }
    ...
    function allowIssuanceLayer2Seigs(address layer2) external view returns (address rollupConfig, bool allowed) {
    ...
    function totalSupplyOfTon_2() external view returns (uint256 tos) {
        tos = (
            (
                ITON(_ton).totalSupply() - ITON(_ton).balanceOf(_wton) - ITON(_ton).balanceOf(address(0))
                    - ITON(_ton).balanceOf(address(1))
            ) * (10 ** 9)
        ) + (_tot.totalSupply());
    }
    ...
    function _uncommittedStakeOf(address layer2, address account) internal view returns (uint256) {
        ...
        uint256 uncommittedBalance = FullMath.rmul(FullMath.rdiv(coinage.balanceOf(account), prevFactor), newFactor);
        ...
        returns (uint256 totAmount)
    {
        ...
        return FullMath.rdiv(FullMath.rmul(totBalalnce - coinageTotalSupply, amount), coinageTotalSupply);
    }
            ...
            if (allowed && !_isPauseL2Seigniorage(msg.sender)) {
                ...
                if (oldLayer2Info.startBlock == 0) {
                    ...
                    if (oldLayer2Info.layer2Tvl > 0) {
                        layer2Seigs =
                            ((l2RewardPerUint * oldLayer2Info.layer2Tvl) / WEI_UNIT) - oldLayer2Info.initialDebt;
    ...
    function _totalSupplyOfTon(uint256 blockNumber) internal view returns (uint256 tos) {
        ...
        tos = initial + (_seigPerBlock * (blockNumber - startBlock)) - (ITON(_ton).balanceOf(address(1)) * (10 ** 9))
            - burntAmount;
    }
