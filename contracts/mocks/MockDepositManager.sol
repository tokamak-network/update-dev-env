...
contract MockDepositManager {
    ...
    mapping(address => mapping(address => WithdrawalReqeust[]))
        public _withdrawalRequests;
    ...
    mapping(address => mapping(address => uint256))
        internal _withdrawalRequestIndex;
    ...
    function deposit(address layer2, uint256 amount) external returns (bool) {
        require(
            _deposit(layer2, msg.sender, amount, msg.sender),
            "fail deposit"
        );
        return true;
    }
    ...
    function processRequests(
        ...
        for (uint256 i = 0; i < n; i++) {
            require(
                _processRequest(layer2, receiveTON),
    ...
    function _deposit(
        ...
        require(
            account != address(0) && amount != 0,
            "zero amount or zero address"
        );
    ...
    function _requestWithdrawal(
        ...
        _pendingUnstaked[layer2][msg.sender] =
            _pendingUnstaked[layer2][msg.sender] +
            amount;
        _pendingUnstakedLayer2[layer2] =
            _pendingUnstakedLayer2[layer2] +
            amount;
        _pendingUnstakedAccount[msg.sender] =
            _pendingUnstakedAccount[msg.sender] +
            amount;
    ...
    function _processRequest(address layer2, bool) internal returns (bool) {
        ...
        require(
            _withdrawalRequests[layer2][msg.sender].length > index,
            "DepositManager: no request to process"
        );

        WithdrawalReqeust storage r = _withdrawalRequests[layer2][msg.sender][
            index
        ];

        require(
            r.withdrawableBlockNumber <= block.number,
        ...
        _accUnstaked[layer2][msg.sender] =
            _accUnstaked[layer2][msg.sender] +
            amount;
        _accUnstakedLayer2[layer2] = _accUnstakedLayer2[layer2] + amount;
        _accUnstakedAccount[msg.sender] =
            _accUnstakedAccount[msg.sender] +
            amount;
